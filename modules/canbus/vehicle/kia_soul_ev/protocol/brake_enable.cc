...
const int32_t BrakeEnable::ID = 0x70;

uint32_t BrakeEnable::GetPeriod() const {
  // receive rate??
...
bool BrakeEnable::NeedSend() {
  bool tmp = send_once_;
  send_once_ = false;
  return tmp;
}

void BrakeEnable::UpdateData(uint8_t *data) {

    oscc_brake_enable_s enable_cmd;
