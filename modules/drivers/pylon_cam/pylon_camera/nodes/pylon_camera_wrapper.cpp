...
namespace pylon_camera
{
    ...
    PylonCamWrapper::PylonCamWrapper(ros::NodeHandle node, ros::NodeHandle priv_node_)
            : nh_(node), priv_node_(priv_node_),
              ...
              grab_imgs_raw_as_(
                      nh_,
                      "grab_images_raw",
                      boost::bind(&PylonCamWrapper::grabImagesRawActionExecuteCB,
                                  this,
                                  _1),
                      false),
    ...
    void PylonCamWrapper::init()
    {
      ...
      pylon_camera_parameter_set_.readFromRosParameterServer(priv_node_);
    ...
    bool PylonCamWrapper::initAndRegister()
    {
    ...
    bool PylonCamWrapper::startGrabbing()
    {
      ...
      for ( int i = 0; i < set_user_output_srvs_.size(); ++i )
      {
        ...
        set_user_output_srvs_.at(i) =
                nh_.advertiseService< camera_control_msgs::SetBool::Request,
                        ...
                        srv_name,
                        boost::bind(&PylonCamWrapper::setUserOutputCB,
                                    this,
                                    i,
                                    _1,
                                    _2));
    ...
    void PylonCamWrapper::setupRectification()
    {
      if ( !img_rect_pub_ )
      {
      ...
      if ( !grab_imgs_rect_as_ )
      {
        grab_imgs_rect_as_ =
                new GrabImagesAS(nh_,
                                 "grab_images_rect",
                                 boost::bind(
                                         &PylonCamWrapper::grabImagesRectActionExecuteCB,
                                         this,
                                         _1),
                                 false);
    ...
    uint32_t  PylonCamWrapper::getNumSubscribersRaw() const
    {
      return ((CameraPublisherLocal*)(&img_raw_pub_))->impl_->image_pub_.getNumSubscribers();
    }

    bool PylonCamWrapper::spin()
    {
    ...
    void PylonCamWrapper::spin_camera()
    {
    ...
    bool PylonCamWrapper::grabImage()
    {
    ...
    void PylonCamWrapper::grabImagesRawActionExecuteCB(
    ...
    void PylonCamWrapper::grabImagesRectActionExecuteCB(
            const camera_control_msgs::GrabImagesGoal::ConstPtr& goal)
    {
      camera_control_msgs::GrabImagesResult result;
      if ( !camera_info_manager_->isCalibrated() )
    ...
    camera_control_msgs::GrabImagesResult PylonCamWrapper::grabImagesRaw(
            const camera_control_msgs::GrabImagesGoal::ConstPtr& goal,
    ...
    bool PylonCamWrapper::setUserOutputCB(const int output_id,
    ...
    const double& PylonCamWrapper::frameRate() const
    {
      return pylon_camera_parameter_set_.frameRate();
    }

    const std::string& PylonCamWrapper::cameraFrame() const
    {
      return pylon_camera_parameter_set_.cameraFrame();
    }

    uint32_t PylonCamWrapper::getNumSubscribersRect() const
    {
      return camera_info_manager_->isCalibrated() ? img_rect_pub_->getNumSubscribers() : 0;
    }

    uint32_t PylonCamWrapper::getNumSubscribers() const
    {
      return img_raw_pub_.getNumSubscribers() + img_rect_pub_->getNumSubscribers();
    }

    void PylonCamWrapper::setupInitialCameraInfo(sensor_msgs::CameraInfo& cam_info_msg)
    {
 ...
 */
    bool PylonCamWrapper::waitForCamera(const ros::Duration& timeout) const
    {
      bool result = false;
    ...
    bool PylonCamWrapper::setBinningX(const size_t& target_binning_x,
                                      size_t& reached_binning_x)
    {
      boost::lock_guard<boost::recursive_mutex> lock(grab_mutex_);
    ...
    bool PylonCamWrapper::setBinningY(const size_t& target_binning_y,
                                      size_t& reached_binning_y)
    {
      boost::lock_guard<boost::recursive_mutex> lock(grab_mutex_);
    ...
    bool PylonCamWrapper::setBinningCallback(camera_control_msgs::SetBinning::Request &req,
    ...
    bool PylonCamWrapper::setExposure(const float& target_exposure,
                                      float& reached_exposure)
    {
      boost::lock_guard<boost::recursive_mutex> lock(grab_mutex_);
      if ( !pylon_camera_->isReady() )
    ...
    bool PylonCamWrapper::setExposureCallback(camera_control_msgs::SetExposure::Request &req,
    ...
    bool PylonCamWrapper::setGain(const float& target_gain, float& reached_gain)
    {
      boost::lock_guard<boost::recursive_mutex> lock(grab_mutex_);
      if ( !pylon_camera_->isReady() )
      {
    ...
    bool PylonCamWrapper::setGainCallback(camera_control_msgs::SetGain::Request &req,
    ...
    bool PylonCamWrapper::setGamma(const float& target_gamma, float& reached_gamma)
    {
      boost::lock_guard<boost::recursive_mutex> lock(grab_mutex_);
      if ( !pylon_camera_->isReady() )
      {
    ...
    bool PylonCamWrapper::setGammaCallback(camera_control_msgs::SetGamma::Request &req,
    ...
    bool PylonCamWrapper::setBrightness(const int& target_brightness,
                                        int& reached_brightness,
    ...
    bool PylonCamWrapper::setBrightnessCallback(camera_control_msgs::SetBrightness::Request &req,
    ...
    void PylonCamWrapper::setupSamplingIndices(std::vector<std::size_t>& indices,
                                               std::size_t rows,
                                               std::size_t cols,
    ...
    void PylonCamWrapper::genSamplingIndicesRec(std::vector<std::size_t>& indices,
    ...
    float PylonCamWrapper::calcCurrentBrightness()
    {
      boost::lock_guard<boost::recursive_mutex> lock(grab_mutex_);
    ...
    bool PylonCamWrapper::setSleepingCallback(camera_control_msgs::SetSleeping::Request &req,
    ...
    bool PylonCamWrapper::isSleeping()
    {
      return is_sleeping_;
    }

    PylonCamWrapper::~PylonCamWrapper()
    {
      if ( pylon_camera_ )
      {
        delete pylon_camera_;
    ...
    void PylonCamWrapper::set_camera_parameters(const ros::NodeHandle &nh) {
      pylon_camera_parameter_set_.readFromRosParameterServer(nh);
    }
