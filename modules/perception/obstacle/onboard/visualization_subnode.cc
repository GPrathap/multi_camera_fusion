...
bool VisualizationSubnode::InitInternal() {
  ...
  AINFO << "lidar_event_id_: "<< lidar_event_id_ << " fusion_event_id_: " << fusion_event_id_;
...
bool VisualizationSubnode::InitStream() {
  ...
  iter = reserve_field_map.find("lidar_event_id");
  if (iter == reserve_field_map.end()) {
    AWARN << "Failed to find lidar_event_id_: " << reserve_;
    lidar_event_id_ = -1;
  } else {
